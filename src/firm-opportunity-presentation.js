export function describeFirmOpportunity(opportunity) {
  ...
  const evidence = `${opportunity.observedDays}/${opportunity.requiredObservationDays} days observed · ${opportunity.latestPotentialCustomers} potential customers today · ${opportunity.expectedDailyRevenue.toFixed(1)} expected daily revenue · ${opportunity.expectedDailyCost.toFixed(1)} expected daily cost`;
