...
export class ShadowCitizenPolicy implements CitizenPolicy {
  ...
  decide(input: CitizenPolicyInput): CitizenPolicyDecision {
    ...
    if (["health", "education", "clinical-care", "mutual-aid-offer", "mutual-aid-receive"].includes(input.observation.kind)) return { ...active, policy: this.activePolicy.id } as CitizenPolicyDecision;
...
export class GatedNeuralCitizenPolicy implements CitizenPolicy {
  ...
  decide(input: CitizenPolicyInput): CitizenPolicyDecision {
    ...
    if (["health", "education", "clinical-care", "workday-plan", "sleep", "mutual-aid-offer", "mutual-aid-receive"].includes(input.observation.kind)) return { ...fallback, policy: this.fallbackPolicy.id } as CitizenPolicyDecision;
    const inference = this.neuralPolicy.infer(input.observation, input.legalActions);
