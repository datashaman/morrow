...
const simulation = new TownSimulation({ latentFirmNames: DEFAULT_LATENT_FIRM_NAMES, housingCapacityEnabled: true, transportEnabled: true, schedulesEnabled: true, sleepEnabled: true, cooperationMode: "mutual-aid", welfareMode: "combined", lifecycleEnabled: true });
...
function updateInterface() {
  ...
  const scheduleEvidence = citizenScheduleEvidence({
    person,
    employer: person.employer >= 0 ? simulation.firms[person.employer] : null,
    day: simulation.day,
    block: state.block,
    scheduledToday: person.employer >= 0 && simulation.scheduledForShift(person, simulation.firms[person.employer]),
