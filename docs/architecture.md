...
`TownSimulation` owns people, their versioned knowledge and learning histories, configured archetypes, runtime firm instances, opportunity observations, contracts, treasury, policy, time, and recent transfer flows. The browser owns selected-person state, selected-firm identity, playback timing, pause state, speed, and canvas dimensions. Runtime firms and contracts may be appended but are never removed, so their numeric IDs remain valid references and their unique `archetypeId:instanceNumber` identities preserve lifecycle history. Historical instances remain inspectable in the firm grid; the canvas suppresses an inactive historical landmark when an active replacement of the same archetype occupies that location.
