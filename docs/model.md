...
#### Experimental public-realm contract

The headless `publicWorksEnabled` option defaults to disabled. When enabled, settlement may form Morrow Civic Works once if at least 20% of living adults are unemployed, two unemployed adults are available, and the treasury can transfer exactly 30 startup cash while retaining 60. The transfer is matched on treasury and firm ledgers, creates no money, and the two formation jobs use ordinary employment, rota, payroll, tax, attendance, maintenance, staffing, owner, distress, and insolvency rules. A closed instance does not re-enter in this tracer.

On each Monday-to-Friday procurement phase, the treasury requests two whole public-realm service units. Attended workers provide at most one unit each before schedule equivalence and operational readiness, capped by the two-unit contract. The treasury must afford the complete deliverable batch; otherwise no cash or service moves. A successful exact payment becomes ordinary firm sales and therefore supports staffing, while payroll tax returns part of it to the treasury. Public procurement occurs before the shared welfare envelope is snapshotted, so it can reduce same-day welfare capacity. The 20% gate, 30 startup transfer, 60 reserve, two jobs, two service units, and 9.5 unit price are falsifiable gameplay hypotheses, not a claim about actual public employment or procurement.

`npm run evaluate:public-works` compares disabled and enabled full-lifecycle arms with deterministic replay. The initial five-seed, 196-day tracer created two jobs in every enabled arm and improved aggregate first-wage reach and employment, but failed its survival and care gates: four seeds had lower food-delivery rates, three had more deaths, and one became extinct 21 days earlier. Observed welfare flow fell by 83.2–490.0 while public spending was 182.0–676.0. These are paired gameplay observations rather than empirical or isolated causal estimates. The result keeps browser activation disabled and preserves the lifecycle activation gate.
...
The map visualizes this employment state without changing it. A current primary activity takes precedence: firm-owned activities gather around that firm, public social activity uses the Common Park, and home-based activity uses the citizen's home or the park when unhoused. Otherwise, an unemployed adult with a current valid application waits outside that employer; unemployed adults without one mill slowly inside the central Common Park. Employed idle adults and dependents return home when housed. The selected-citizen panel and canvas accessibility text expose the reason from the same map-presence result used to choose the rendered target. The renderer reads `jobApplicationFirm` but never creates an application or affects candidate ranking, vacancy age, acceptance, hiring, housing, or any need.
