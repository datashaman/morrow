...
Seven firm archetypes are configured in `src/config.js`. The interactive town starts with the four-firm essential foundation—Harvest Foods, HomeWorks, Makers Guild, and Morrow Fields—while Common Café, Green Basket, and Morrow Apothecary begin as latent opportunities. The explicit six-firm initialization keeps the new apothecary latent in headless diagnostics so historical neural gates and viability comparisons retain their original scenario.
...
#### Private café, premium-food, and apothecary formation

Endogenous private formation applies when the Common Café, Green Basket, or Morrow Apothecary archetype is absent at initialization. An absent archetype remains off the map and appears in the firm panel as an opportunity rather than an operating firm.
