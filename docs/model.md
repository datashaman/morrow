...
Eight firm archetypes are configured in `src/config.js`. The interactive town starts with the four-firm essential foundation—Harvest Foods, HomeWorks, Makers Guild, and Morrow Fields—while Common Café, Green Basket, Morrow Apothecary, and Morrow School begin as latent opportunities. Historical viability diagnostics explicitly restrict their formation set to the original café and premium grocer so earlier comparisons retain their scenario.
...
#### Private café, premium-food, apothecary, and school formation

Endogenous private formation applies when the Common Café, Green Basket, Morrow Apothecary, or Morrow School archetype is absent at initialization. An absent archetype remains off the map and appears in the firm panel as an opportunity rather than an operating firm.
...
A living person below 72% skill also receives a separate education choice when an operating school has lesson inventory. Purchasing is legal only when the person can pay 4.5 exactly and retain three current essential-cost days. `motivation-v3` weighs mastery, planning, the skill gap, safety, stress, avoidance, and runway. A lesson consumes one finite teaching slot, raises skill by 0.01 up to 0.95, and raises growth by 0.015. It changes later reservation wages and employer ranking only through the existing skill rules; it never creates or guarantees a vacancy, application, offer, or job.
