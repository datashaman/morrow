...
Individual ledger records store the day, per-person activity sequence, direction, amount, purpose, and cash balance before and after. Life events carry the same sequence so the interface can combine both record types into one newest-first activity stream. Firm streams also merge structured knowledge-effect records. The stream defaults to all activity and can be filtered to transactions, life events, or knowledge effects where applicable. The selected filter survives firm switching. The model retains the complete history for the current in-memory run; the interface renders all matching entries inside a scrollable region and preserves the reader's position while new activity arrives.
