...
Morrow School's current finite lesson is an explicit retail-operations course. A completed paid lesson retains the migration-era generic skill gain of `0.01` up to `0.95`, mirrors that result into `general`, adds `0.04 × (1 − retail)` retail knowledge, and transfers `0.01 × (1 − inventory)` inventory knowledge. Each domain change is recorded separately in the same complete learning history with the school as its education source. The course rates and cross-domain transfer are design hypotheses; they are not credentials or calibrated evidence of learning.
...
The knowledge tracer changes only the everyday grocer. Its scalar daily capacity remains `floor(attending workers × configured base × maintenance readiness)`. After attendance and that day's learning, each attending grocery worker also contributes `configured base × 0.15 × mean(retail, inventory) × maintenance readiness` to a firm-level fractional accumulator. Whole accumulated points become extra transaction slots for that day; only the fractional remainder below one carries forward. Unused whole slots expire during settlement. Zero vocational knowledge exactly preserves scalar capacity, while fully bounded knowledge adds at most 15% over time. Because carry releases a prior fraction, a particular day's integer result may be one slot above its instantaneous fractional contribution. No other current firm's transaction capacity reads knowledge.

Accrual runs exactly once per production day and ordinary capacity reads are pure. A day that releases one or more slots creates one firm life event with the slot count and remaining carry. The firm panel shows today's knowledge slots and carry even when both are zero. Carry and slot accounting move no cash or inventory and cannot create demand; an additional slot changes sales only when a customer would otherwise have been turned away for capacity.
...
Firms and the treasury use the same sequenced ledger and event shape. One selected-firm dossier itemizes the chosen firm's output, production or sourcing method, upstream producer, cash, inventory, staffing evidence, smoothed net income, vital status, rescue history, and current lifecycle state. Its dropdown retains closed historical instances and stays synchronized with map selection. Construction processors additionally show awaiting input stock, stocked output, scalar processing capacity, units processed today, and labor-limited shortfall; their activity history records processed quantities and whether absence or exhausted capacity retained inputs. Supply buyers also show today's requested and delivered contract quantities. The dossier exposes the complete combined transaction and lifecycle-event stream with the same retained filter and scrolling behavior as a citizen.
...
Morrow Builders applies the same scalar rule: a delivered 16 bundle remains explicit input until attending builders convert it one-for-one into a stocked project. No attendance produces no output; partial readiness floors capacity to whole units; unused capacity expires; and every unprocessed input remains owned by the receiving firm. Processing moves no cash and vocational knowledge provides no bonus in this baseline. The one-unit worker rates and readiness multiplier are gameplay hypotheses rather than calibrated construction productivity.
...
A citizen has a deterministic reserve target of one, two, or three meals. When their reserve is empty, the simulation creates exact purchase options for every affordable quantity from one portion through that target, bounded by each active food firm's inventory. Each option records seller, portions, unit and total price, FIFO effective quality, remaining shelf life, and whether attending workers currently have transaction capacity. Skipping food is always legal. When food is already stored, its oldest viable meal becomes the legal consumption option with seller, age, effective quality, and remaining life.
...
The browser starts with neural control disabled. A visible switch can enable it without resetting the town only because the matching artifact passed the versioned personal-time activation gate. When enabled, the neural choice controls the schema-v2 discretionary personal-time action only. The observation now includes general, retail, and inventory knowledge, but the migrated bundled artifact gives those three inputs zero weight and therefore retains its pre-migration behavior. `motivation-v3` still controls attendance, job search and offers, food, housing, self-care, clinical care, education, and all owner decisions. Health, education, and clinical-care decisions remain outside neural shadow inference until a future schema and gate explicitly cover them. `TownSimulation` continues to generate and validate legal actions and apply consequences. Disabling the switch returns the very next controlled decision to `motivation-v3` without changing town state. See [neural activation](./neural-activation.md) for the gate evidence and its limits.
