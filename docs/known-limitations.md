...
- After dated spoilage was introduced, the employment evaluator's locked six-seed control baseline changed and the existing first-wage gate no longer passes, while its funded-opportunity and mortality gates still pass. The versioned schedule-disabled evaluator retains that failure rather than weakening its acceptance threshold; closure-aware planning remains isolated to schedule-enabled interactive runs until an integrated evaluator slice establishes a new baseline.
